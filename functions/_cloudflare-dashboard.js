...
export async function cloudflareFetch(path, config, query = {}) {
...
async function enrichZone(zone, config, index) {
  ...
  const siteStatus = !zoneHealthy ? "degraded" : probe.ok ? "up" : "down";
...
function buildCloudflareMetrics({ verify, zones, workers }) {
  ...
  return [
    { label: "Status", value: verify.ok ? "Active" : "Error", tone: verify.ok ? "good" : "danger", trend: trendFromCount(12) },
    { label: "DNS records", value: String(dnsCount), tone: "good", trend: trendFromCount(dnsCount, 1) },
    { label: "Proxied records", value: String(proxiedCount), tone: proxiedCount ? "good" : "warn", trend: trendFromCount(proxiedCount, 2) },
    { label: "Active zones", value: `${activeZones}/${zoneCount}`, tone: activeZones === zoneCount ? "good" : "warn", trend: trendFromCount(activeZones, 3) },
    { label: "Workers", value: workerCount === null ? "n/a" : String(workerCount), tone: workerCount === null ? "warn" : "good", trend: trendFromCount(workerCount || 1, 4) }
  ];
}
...
function buildAlerts({ verify, zones, permissions }) {
  ...
  if (!verify.ok) push("critical", "Cloudflare API token failed verification", "Check CLOUDFLARE_API_TOKEN in Pages secrets.");
  Object.entries(permissions).forEach(([key, allowed]) => {
    if (!allowed) push("warning", `Missing ${key} API permission`, "Some dashboard data is unavailable for this token.");
  });
...
function buildSummary({ zones, workers }) {
  ...
  return {
    dns: { value: totalDns, label: "DNS records", detail: `${proxiedRecords} proxied`, series: dnsByZone.length ? dnsByZone : [0] },
    response: { value: avgResponse, label: "Avg. live response", detail: `${responseByZone.length}/${zones.length} zones reachable`, series: responseByZone.length ? responseByZone : [0] },
    workers: { value: Array.isArray(workers) ? workers.length : null, label: "Workers services" }
  };
}
...
export function notConfiguredResponse(config) {
  return {
    configured: false,
    ok: false,
    accountId: maskAccountId(config.accountId),
    apiStatus: !config.accountId ? "Account ID missing" : "Token needed",
    message: "Set CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN as Cloudflare Pages environment variables."
  };
}
